        ...
        <div className="relative z-10 flex flex-col items-center gap-8 px-6 w-full max-w-lg animate-fade-in">
          ...
          <div className={`w-full rounded-2xl border px-6 py-5 flex flex-col gap-3 ${rpGained > 0 ? "border-[#4af]/30 bg-[#4af]/5" : "border-white/10 bg-white/4"}`}>
            ...
            {prevRP !== newRP && !rankChanged && (
              <p className="text-white/30 text-xs">Önceki RP: {prevRP} → Yeni RP: {newRP}</p>
            )}
          </div>

          {/* Butonlar */}
          <div className="flex flex-col gap-3 w-full">
            {/* Paylaş butonu — premium gradient */}
            <button
              onClick={() => setShowShare(true)}
              className="w-full py-3.5 rounded-2xl flex items-center justify-center gap-2.5 font-bold text-sm tracking-wide transition-all active:scale-[0.98]"
              style={{
                background: `linear-gradient(135deg, ${rank.tier.color}30, ${rank.tier.color}18)`,
                border: `1px solid ${rank.tier.color}40`,
                color: rank.tier.color,
                boxShadow: `0 0 20px ${rank.tier.color}15`,
              }}
            >
              <Share2 size={16} />
              Sonuç Kartını Paylaş
            </button>
