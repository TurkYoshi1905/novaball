...
export function useMultiplayerPhysics({
  ...
  const buildGR = useCallback((): GR => {
    ...
    for (const p of players) {
      ...
      charges[p.username] = 0; cooldowns[p.username] = 0; goalCounts[p.username] = 0;
    }
    return {
      players, ball: { x: CANVAS_WIDTH / 2, y: CANVAS_HEIGHT / 2, vx: 0, vy: 0 },
      score: { red: 0, blue: 0 },
      gameTimeMs: ranked ? RANKED_DURATION_MS : 0,
      phase: "playing", goalPauseEnd: 0, goalCounts, lastGoalTeam: null,
      inputs, charges, cooldowns, lastSync: 0, lastInputBc: 0,
    };
  }, [match, ranked]);
  ...
  const draw = useCallback((gr: GR, canvas: HTMLCanvasElement) => {
    ...
    ctx.strokeStyle = "rgba(100,200,100,0.15)"; ctx.lineWidth = 1.5; ctx.stroke();
    ctx.beginPath(); ctx.arc(cx, cy, 80, 0, Math.PI * 2);
    ctx.strokeStyle = "rgba(100,200,100,0.15)"; ctx.stroke();

    const boxH = 160, boxW = 60, boxTop = cy - boxH / 2;
    ctx.strokeStyle = "rgba(100,200,100,0.10)"; ctx.lineWidth = 1;
    ctx.strokeRect(FIELD_LEFT, boxTop, boxW, boxH);
    ctx.strokeRect(FIELD_RIGHT - boxW, boxTop, boxW, boxH);

    // Kaleler
    const drawGoal = (side: "left" | "right") => {
      const x = side === "left" ? FIELD_LEFT - GOAL_DEPTH : FIELD_RIGHT;
      ctx.fillStyle   = side === "left" ? "rgba(68,136,255,0.10)" : "rgba(230,53,53,0.10)";
      ctx.fillRect(x, GOAL_TOP, GOAL_DEPTH, GOAL_HEIGHT);
      ctx.strokeStyle = side === "left" ? "rgba(68,136,255,0.45)" : "rgba(230,53,53,0.45)";
      ctx.lineWidth = 2; ctx.strokeRect(x, GOAL_TOP, GOAL_DEPTH, GOAL_HEIGHT);
    };
    drawGoal("left"); drawGoal("right");

    const posts = [
      { x: FIELD_LEFT, y: GOAL_TOP }, { x: FIELD_LEFT, y: GOAL_BOTTOM },
    ...
    for (const p of gr.players) {
      ...
      // Stamina barı
      const barW = PLAYER_RADIUS * 2, barX = p.x - PLAYER_RADIUS, barY = p.y + PLAYER_RADIUS + 5;
    ...
    const { ball } = gr;
    ctx.save();
    ctx.shadowBlur = 18; ctx.shadowColor = "#ffffff70";
    ctx.beginPath(); ctx.arc(safe(ball.x), safe(ball.y), BALL_RADIUS, 0, Math.PI * 2);
  ...
  const physicsStep = useCallback((gr: GR, dt: number) => {
    ...
    for (const p of gr.players) {
      ...
      if (inp.shoot) {
        ...
        const dx = gr.ball.x - p.x, dy = gr.ball.y - p.y;
        const d  = Math.max(0.001, Math.sqrt(dx * dx + dy * dy));
        if (d < KICK_RANGE) {
          gr.ball.vx += (dx / d) * power; gr.ball.vy += (dy / d) * power;
    ...
    // Top duvar çarpışmaları
    const ball = gr.ball;
    if (ball.y - BALL_RADIUS < FIELD_TOP)    { ball.y = FIELD_TOP + BALL_RADIUS;    ball.vy =  Math.abs(ball.vy) * BALL_RESTITUTION; }
    if (ball.y + BALL_RADIUS > FIELD_BOTTOM) { ball.y = FIELD_BOTTOM - BALL_RADIUS; ball.vy = -Math.abs(ball.vy) * BALL_RESTITUTION; }
    ...
    if (ball.x - BALL_RADIUS < FIELD_LEFT) {
      ...
      } else { ball.x = FIELD_LEFT + BALL_RADIUS; ball.vx = Math.abs(ball.vx) * BALL_RESTITUTION; }
    }
    ...
    if (ball.x + BALL_RADIUS > FIELD_RIGHT) {
      ...
      } else { ball.x = FIELD_RIGHT - BALL_RADIUS; ball.vx = -Math.abs(ball.vx) * BALL_RESTITUTION; }
    }
    ...
    ball.x = safe(ball.x + ball.vx); ball.y = safe(ball.y + ball.vy);
  ...
  useEffect(() => {
    ...
    if (isHost) {
      onPlayerInput(ch, inp => {
        if (grRef.current) grRef.current.inputs[inp.username] = { dx: inp.dx, dy: inp.dy, shoot: inp.shoot, sprint: inp.sprint };
      });
    } else {
    ...
    const loop = (ts: number) => {
      ...
      const lp = g.players.find(p => p.username === localUsername);
      if (lp) lp.kickCharge = g.charges[localUsername] ?? 0;
      ...
      if (isHost) {
        ...
        if (now - g.lastInputBc >= INPUT_MS) {
          g.lastInputBc = now;
          const inp: PlayerInput = { username: localUsername, dx: g.inputs[localUsername]?.dx ?? 0, dy: g.inputs[localUsername]?.dy ?? 0, shoot, sprint, ts: now };
  ...
  return { score: hudScore, gameTimeMs: hudTime, phase: hudPhase, lastGoalTeam: hudLastGoalTeam };
}
